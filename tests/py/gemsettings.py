...
class GemSettings(structy.Struct):
    ...
    castor_knob_min: structy.Fix16 = structy.Fix16(-1.01)
    """Configuration for the CV knob mins and maxs in volts, defaults to -1.01 to +1.01."""

    castor_knob_max: structy.Fix16 = structy.Fix16(1.01)

    pollux_knob_min: structy.Fix16 = structy.Fix16(-1.01)

    pollux_knob_max: structy.Fix16 = structy.Fix16(1.01)

    chorus_max_intensity: structy.Fix16 = structy.Fix16(0.05)
    """Maximum amount that the chorus can impact Pollux's frequency."""

    chorus_max_frequency: structy.Fix16 = structy.Fix16(0.2)
    """Maximum frequency that the chorus can run at in hertz."""

    knob_offset_corr: structy.Fix16 = structy.Fix16(0.0)
    """Error correction for the ADC readings for the CV and PWM knobs."""

    knob_gain_corr: structy.Fix16 = structy.Fix16(1.0)

    smooth_initial_gain: structy.Fix16 = structy.Fix16(0.1)
    """Pitch input CV smoothing parameters."""

    smooth_sensitivity: structy.Fix16 = structy.Fix16(30.0)
