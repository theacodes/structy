...
import structy.kinds
import structy.parser
import structy.templates
...
def relativize_from_common_path(*paths):
    ...
    try:
        ...
        return [pathlib.Path(common_path), *(pathlib.Path(path).relative_to(common_path) for path in paths)]
    except ValueError:
        return paths
...
def run(language : str, struct_file, destination_path : pathlib.Path):
    template = structy.templates.template_for(language)

    if not destination_path.exists():
        if os.isatty():
            create = input(f"Hey, {destination_path} doesn't exist, do want me to create it? (y/n)")
            if create == "y":
    ...
    if not destination_path.is_dir():
        raise StructyError(f"Hey, {destination_path} isn't a path. I can't put files into another file.")

    source_file_name = pathlib.Path(struct_file.name).name
    source_file_stem = pathlib.Path(struct_file.name).stem

    struct = structy.parser.parse(struct_file.read())
